...
const REPO_ROOT = "C:/Users/User/projects/soc-career-development-handbook";
...
// BOOK-INDEX.md is a big Markdown "Part Table" grouped by "### Section X — Name"
// subheadings, same shape as the Detection Engineering Handbook's and SOC Manager's
// Operating Handbook's indexes. This book's Appendix Table lists 7 appendices
// (A1-A7) that have not been drafted yet - appendices/ is empty - so parsing
// deliberately stops at "## Appendix Table" rather than emitting 7 MISSING FILE
// placeholders into a book whose appendices simply don't exist yet. Re-enable
// appendix parsing (delete the early return below) once appendices/ has content.
function parseIndex() {
  ...
  for (const raw of lines) {
    ...
    if (line === "## Appendix Table") {
      break; // appendices not drafted yet for this release - stop here
    }
    ...
    if (line.startsWith("|")) {
      ...
      // split, then restore, same as the sibling books' parsers.
...
// This book's seven content tags (STYLE-GUIDE.md §... "Multi-level content model"),
// distinct from the org-side tags in the SOC Manager's Operating Handbook and the
// technical tags in the Detection Engineering Handbook, but reusing this series'
// established color logic where the parallel is real: aqua for the entry-level tag,
// blue for the senior/specialist tag, orange for the leadership tag.
const TAG_COLORS = {
  CONCEPT: "tag-concept",
  "L1/L2": "tag-l1-l2",
  "SENIOR/SPECIALIST": "tag-senior-specialist",
  "LEAD/MANAGEMENT TRACK": "tag-lead-management",
  "STUDY PLAN": "tag-study-plan",
  "INTERVIEW PREP": "tag-interview-prep",
  MINDSET: "tag-mindset",
};
...
function main() {
  ...
  * { box-sizing: border-box; }
  ...
  .tag-l1-l2 { background: var(--aqua-tint); color: #0d6b49; border: 0.5pt solid var(--aqua); }
  .tag-senior-specialist { background: var(--blue-tint); color: #164a8a; border: 0.5pt solid var(--blue); }
  .tag-lead-management { background: var(--orange-tint); color: #93401d; border: 0.5pt solid var(--orange); }
  .tag-study-plan { background: var(--violet-tint); color: #2e2470; border: 0.5pt solid var(--violet); }
  .tag-interview-prep { background: var(--magenta-tint); color: #99245c; border: 0.5pt solid var(--magenta); }
  .tag-mindset { background: var(--green-tint); color: #0a5c0a; border: 0.5pt solid var(--green); }
  `;
...
<head>
<meta charset="utf-8">
<title>The SOC Career Development Handbook</title>
<style>${css}</style>
</head>
<body>
<section class="cover">
  <div class="kicker">NESHBOY SOC Professional Library</div>
  <h1>THE SOC CAREER DEVELOPMENT HANDBOOK</h1>
  <div class="accent-rule"></div>
  <h2>Study Plans, Home-Lab Projects, Portfolios, and Interview Prep — The Analyst's Own Climb From L1 to SOC Architect</h2>
