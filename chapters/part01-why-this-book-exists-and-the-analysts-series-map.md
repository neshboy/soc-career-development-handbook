...
**[SENIOR/SPECIALIST]** L3 and the branch point are where the SOC Manager's Operating Handbook, Part 13 §3's three named branches — detection engineer, threat hunter, and team lead — plus its unlabeled fourth road (§3.4, staying a strong individual contributor) become real choices instead of abstract titles, alongside incident responder, this book's own added specialization track (Part 17) with no counterpart in that organizational ladder, and where a portfolio built at senior analyst needs to actually work for whichever road you pick.
...
**[SENIOR/SPECIALIST]** Section F (Parts 15–18) is the branch point itself, ordered detection engineer, threat hunter, incident responder, then staff/principal individual contributor. The detection-engineer and threat-hunter order matches the SOC Manager's Operating Handbook, Part 13 §3's own order for those two branches exactly, so a reader moving between the two books never has to remap which comes first; incident responder is this book's own added specialization track, with no counterpart in Part 13's organizational ladder, and staff/principal individual contributor closes the section per Part 13 §3.4's unlabeled fourth road. (Part 13 §3's third named branch, team lead, is covered separately in this book's Section G.) Each part builds toward the specific work-sample bar that part of the SOC Manager's Operating Handbook names — five merged detections with a tracked false-positive rate for the detection-engineer branch, two structured hunts with a stated hypothesis and abandonment condition for the hunter branch, a self-run tabletop and a DFIR home-lab build for incident response — and cites Detection Engineering Handbook V2 for the technical methodology behind each, rather than re-teaching query languages or hunt mechanics this book has no business re-deriving.
